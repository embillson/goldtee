...
export default function ProductCard({
  ...
  return (
    <div className="group bg-[var(--surface)] rounded-2xl overflow-hidden border border-[var(--border)] hover:border-[var(--gold)]/40 transition-all duration-300 flex flex-col">
      ...
      {/* Info */}
      <div className="p-5 flex flex-col gap-4 flex-1 text-center items-center">
        <h3 className="font-semibold text-[var(--white)] text-sm leading-snug">
          {name}
        </h3>
        ...
        <button
          onClick={handleAdd}
          disabled={stock === 0}
          className="w-full bg-[var(--gold)] text-[var(--dark)] text-sm font-semibold py-2.5 rounded-full hover:bg-[var(--gold-light)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed mt-auto"
        >
